...
def main(args):
        ...
        for run_id, (run, stdin, stdout, stderr) in enumerate(
            ...
            cmds.append(
                (
                    ...
                    bench,
                    run_id,
                    f"{cmd_stdin}{cmd_stdout}{cmd_stderr}",
                )
            )
    ...
    with open(args.out_path, "w") as out_file:
        ...
        for cmd, options, bench, run_id, extras in cmds:
            ...
            final_cmd = f'time make spec SPECOUTDIR={outdir} SPECCMD={cmd} SPECOPTIONS="{options}" {extras} 2>&1 > {outdir}/log &'
